...
var line = (bend = 0, warp = 0) => {
  ...
  let cubicA = interpolate(Math.abs(bend), Empty, cubicB);
  if (bend !== 0 && warp > 0) {
    if (bend > 0) {
      cubicA = interpolate(warp, cubicA, { x: 0, y: cubicB.x * 2 });
    } else {
      cubicA = interpolate(warp, cubicA, { x: cubicB.y * 2, y: 0 });
    }
  }
  const bzr = cubic(
    Empty,
...
//# sourceMappingURL=chunk-V7NDADG7.js.map