import {
  points,
  uniqueInstances
} from "./chunk-4VLG2RRD.js";
import {
  Forms_exports
} from "./chunk-EPBWU3RU.js";
import {
  CanvasHelper
} from "./chunk-LSH4TO74.js";
import {
  DomRx_exports,
  ElementSizer,
  ...
  setCssDisplay,
  setCssToggle
} from "./chunk-F5ILIR7Y.js";
import {
  Elements_exports
} from "./chunk-ALBZ3JYS.js";
import {
  shortGuid
...
var setFromVariables = (context, ...options) => {
  ...
  for (const opt of options) {
    ...
    if (`query` in opt && opt.query !== void 0) {
      ...
      query2 = `#${opt.id}`;
    } else if (`object` in opt && opt.object !== void 0) {
    ...
    if (opt.attribute) {
      for (const el2 of els) {
        if (isHtmlElement(el2)) {
          el2.setAttribute(opt.attribute, v);
        } else {
          throw new Error(`Trying to set an attribute on something not a HTML element`, el2);
        }
      }
    } else if (opt.field) {
      for (const el2 of els) {
        if (typeof el2 === `object`) {
          ...
          throw new Error(`Trying to set field on something that is not an object (${typeof el2})`, el2);
        }
      }
    } else {
...
var draggable = (elemOrQuery, listener, options = {}) => {
  ...
  let initialPointerPosition = point_exports.Placeholder;
  let token;
  ...
  let fenceOffset = rect_exports.PlaceholderPositioned;
  ...
  const validateOffsetAndPoint = (offset, x, y) => {
    if (!rect_exports.isPlaceholder(fenceOffset)) {
      offset = {
  ...
  let lastMoveOffset = point_exports.Empty;
  const onPointerMove = (moveEvent) => {
    moveEvent.preventDefault();
    moveEvent.stopPropagation();
    let { x, y } = moveEvent;
    let offset = point_exports.isPlaceholder(initialPointerPosition) ? { x: moveEvent.offsetX, y: moveEvent.offsetY } : {
    ...
    if (typeof listener.progress !== `undefined`) {
      ...
      if (p.abort) return onDragCancel(void 0, `discontinued`);
      if (p.viewport) {
        offset = {
  ...
  const onDragCancel = (event, reason = `pointercancel`) => {
    dragCleanup();
    let viewport = point_exports.Placeholder;
...
var pointerVisualise = (elOrQuery, options = {}) => {
  ...
  const trackPointer = async (event) => {
    ...
    const el22 = Elements_exports.circle(
...
var tabSet = (options) => {
  ...
  tabSetEl.querySelector(`fieldset`)?.addEventListener(`change`, (event) => {
    ...
    select(el2.getAttribute(`data-tabset`));
  });
  let currentPanel;
  const select = (id) => {
...
//# sourceMappingURL=chunk-TNV7GB22.js.map