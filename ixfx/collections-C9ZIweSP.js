...
import { throwIntegerTest, throwNumberTest } from "./numbers-BSDDLVnO.js";
import { throwStringTest } from "./sleep-BL2_YkuW.js";
import { toStringAbbreviate } from "./text-W54hKRuZ.js";
import { toStringDefault$2 as toStringDefault } from "./to-string-BizpbVHG.js";
import { addKeepingExisting, filter$2 as filter, find, hasAnyValue, isEqualDefault$2 as isEqualDefault, isEqualValueIgnoreOrder$2 as isEqualValueIgnoreOrder, sortByValueProperty, toArray$2 as toArray } from "./maps-BDdFW1EQ.js";
import { defaultKeyer$2 as defaultKeyer } from "./default-keyer-BtnLRnJ-.js";
import { isPrimitive$2 as isPrimitive } from "./is-primitive-Dqo_ZXEp.js";
import { intervalToMs$2 as intervalToMs } from "./interval-type-CiSB_YRT.js";
import { last as last$1, last$1 as last, map, max$6 as max, min$6 as min } from "./src-uVIhGcK6.js";
import { isEqualDefault$3 as isEqualDefault$1 } from "./is-equal-1lcOs9Gg.js";
...
* @param parameterName
*/
const nullUndef = (value$1, parameterName = `?`) => {
	if (typeof value$1 === `undefined`) return [false, `${parameterName} param is undefined`];
	if (value$1 === null) return [false, `${parameterName} param is null`];
	return [true];
};
const throwNullUndef = (value$1, parameterName = `?`) => {
	const r = nullUndef(value$1, parameterName);
	if (r[0]) return;
	throw new Error(r[1]);
};
...
*/
const circularArray = (capacity) => new CircularArray(capacity);
...
* @returns {(V | undefined)}
*/
const peek$1 = (opts, queue) => queue[0];
const isEmpty$1 = (opts, queue) => queue.length === 0;
const isFull$1 = (opts, queue) => {
...
var QueueMutable = class extends SimpleEventEmitter {
	...
	*/
	...
	dequeue() {
		const v = peek$1(this.options, this.data);
		if (v === void 0) return;
	...
	*/
	...
	get isEmpty() {
		return isEmpty$1(this.options, this.data);
	}
	get isFull() {
		return isFull$1(this.options, this.data);
	}
	get length() {
		return this.data.length;
	}
	get peek() {
		return peek$1(this.options, this.data);
	}
};
/**
...
* @returns
*/
function mutable$3(options = {}, ...startingItems) {
...
* ```
*/
const mutable$2 = (opts = {}, ...startingItems) => new StackMutable({ ...opts }, [...startingItems]);
...
const wrap = (n) => {
	return {
		...
		*queryValue(value$1) {
		...
		addValue: (value$1) => {
			const nodeValue = addValue(value$1, n);
...
const fromPlainObject = (value$1, label = ``, parent, seen = []) => {
	...
	parent = parent === void 0 ? root() : addValue({
		label,
		value: value$1
	}, parent);
...
function* children(node, options = {}) {
	throwNullUndef(node, `node`);
	const filter$1 = options.filter ?? `none`;
	const filterByValue = (v) => {
		if (filter$1 === `none`) return [true, isPrimitive(v)];
		else if (filter$1 === `leaves` && isPrimitive(v)) return [true, true];
		else if (filter$1 === `branches` && !isPrimitive(v)) return [true, false];
...
* @returns
*/
function find$1(root$1, predicate, order = `breadth`) {
...
* @returns
*/
const mutable$1 = (keyString) => new SetStringMutable(keyString);
/**
* Mutable string set
*/
...
var PriorityMutable = class extends QueueMutable {
	...
	*/
	enqueueWithPriority(item, priority$1) {
		throwNumberTest(priority$1, `positive`);
		super.enqueue({
			item,
			priority: priority$1
		});
...
var Table = class {
	...
	* @param value
	*/
	set(rowNumber, columnNumber, value$1) {
		const row = this.getOrCreateRow(rowNumber);
		row[columnNumber] = value$1;
	}
	...
	* @param value
	*/
	setRow(rowNumber, cols, value$1) {
		const row = this.getOrCreateRow(rowNumber);
		for (let columnNumber = 0; columnNumber < cols; columnNumber++) row[columnNumber] = value$1;
	}
};
...
function testGraph(g, parameterName = `graph`) {
	if (g === void 0) return [false, `Param '${parameterName}' is undefined. Expected Graph`];
	...
	} else return [false, `Param '${parameterName} is type '${typeof g}'. Expected an object Graph`];
	return [true];
}
function throwGraphTest(g, parameterName = `graph`) {
...
//#endregion
export { ExpiringMap, MapOfSimpleMutable, QueueImmutable, QueueMutable, SetStringImmutable, SetStringMutable, StackImmutable, StackMutable, Table, circularArray, collections_exports, graph_exports, map_exports, queue_exports, set_exports, stack_exports, tree_exports };
//# sourceMappingURL=collections-C9ZIweSP.js.map