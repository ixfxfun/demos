...
var rgbToColorJs = (rgb) => {
  ...
  return opacity === void 0 ? new Color(`srgb`, coords) : new Color(`srgb`, coords, opacity);
};
var toString = (colour) => {
  const c4 = resolve(colour);
  return c4.display();
...
var fromHsl = (h, s = 1, l = 0.5, opacity = 1) => {
...
var toRgb = (colour) => {
  const c4 = resolve(colour);
  const rgb = c4.srgb;
  return c4.alpha < 1 ? { r: rgb.r, g: rgb.g, b: rgb.b, opacity: c4.alpha, space: `srgb`, unit: `relative` } : { r: rgb.r, g: rgb.g, b: rgb.b, opacity: 1, space: `srgb`, unit: `relative` };
};
...
var toRgb8bit = (rgb) => {
  ...
  if (opacity !== void 0) return { ...t, opacity: opacity * 255 };
  return t;
};
var multiplyOpacity = (colour, amt) => {
  const c4 = resolve(colour);
  const alpha = clamp(c4.alpha * amt);
  c4.alpha = alpha;
  return c4.toString();
};
var multiplySaturation = (colour, amt) => {
  const c4 = resolve(colour);
  console.log(`c.s: ${c4.s}`);
  c4.s = c4.s * amt;
  console.log(`final: ${c4.s}`);
  return c4.toString();
};
...
//# sourceMappingURL=chunk-PSUENZJT.js.map