...
async function* fromArray(array2, interval = 1) {
...
function filter2(input, predicate, options) {
  const upstream = initUpstream(input, {
    ...options,
    onValue(value) {
      if (predicate(value)) {
...
// src/rx/ToArray.ts
async function toArray2(source, options = {}) {
...
async function toArrayOrThrow(source, options = {}) {
  ...
  const v = await toArray2(source, { limit, maximumWait, underThreshold: `partial` });
...
function wrap(source) {
  return {
    ...
    filter: (predicate, options) => {
      return wrap(filter2(source, predicate, options));
    },
    ...
    min: (options = {}) => {
      return wrap(min4(source, options));
    },
    max: (options = {}) => {
      return wrap(max3(source, options));
    },
    average: (options = {}) => {
    ...
    toArray: (options) => {
      return toArray2(source, options);
    },
...
function arrayObject(initialValue = [], options = {}) {
  ...
  const deleteWhere = (filter3) => {
    const valueChanged = value.filter((v) => !filter3(v));
...
var Ops = {
  ...
  */
   ...
   * @returns 
   */
  filter: (predicate) => opify(filter2, predicate),
  /**
  ...
  */
   ...
   * @returns 
   */
  max: (options) => opify(max3, options),
...
//# sourceMappingURL=chunk-RTFJIK63.js.map