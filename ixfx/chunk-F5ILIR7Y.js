...
var CartesianCanvasPlot = class {
  ...
  #drawConnected(dots, colour, width) {
    ...
    ctx.strokeStyle = colour_exports.toString(colour);
    ctx.lineWidth = width;
    ctx.stroke();
    ctx.closePath();
  }
  #drawDot(originalDot, fallbackColour, fallbackRadius) {
    const colour = colour_exports.toString(originalDot.fillStyle ?? fallbackColour);
  ...
  #drawLineCanvasSpace(line2, colour, width, debug = false) {
    ...
    const ctx = this.#canvasRegion.context;
    colour = colour_exports.toString(colour);
    ctx.beginPath();
...
//# sourceMappingURL=chunk-F5ILIR7Y.js.map