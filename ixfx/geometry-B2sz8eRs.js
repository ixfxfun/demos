...
import { ObjectTracker, SimpleEventEmitter, TrackedValueMap, clamp, clampIndex, clamper, guardArray, numberTest, throwIntegerTest, throwNumberTest, throwPercentTest, zipKeyValue } from "./simple-event-emitter-DFseJGjf.js";
...
const dotProduct$3 = (values$1) => {
	...
	for (let index = 0; index < length$4; index++) {
		let t$1 = 0;
		for (const [p, value] of values$1.entries()) if (p === 0) t$1 = value[index];
...
* @returns
*/
const wrap$3 = (v, min$1 = 0, max$1 = 1) => {
	throwNumberTest(v, ``, `min`);
	throwNumberTest(min$1, ``, `min`);
	throwNumberTest(max$1, ``, `max`);
...
const { abs: abs$2, cos: cos$2, sin: sin$2, acos: acos$1, atan2, sqrt: sqrt$2, pow: pow$1 } = Math;
function crt(v) {
	return v < 0 ? -pow$1(-v, 1 / 3) : pow$1(v, 1 / 3);
}
const pi$3 = Math.PI, tau = 2 * pi$3, quart = pi$3 / 2, epsilon = 1e-6, nMax = Number.MAX_SAFE_INTEGER || 9007199254740991, nMin = Number.MIN_SAFE_INTEGER || -9007199254740991, ZERO = {
	x: 0,
	y: 0,
	z: 0
};
const utils = {
	...
	approximately: function(a, b, precision) {
		return abs$2(a - b) <= (precision || epsilon);
	},
	length: function(derivativeFn) {
		...
		for (let i = 0, t$1; i < len; i++) {
			...
			sum$4 += utils.Cvalues[i] * utils.arcfn(t$1, derivativeFn);
		}
		return z * sum$4;
	},
	...
	abcratio: function(t$1, n) {
		...
		const bottom = pow$1(t$1, n) + pow$1(1 - t$1, n), top = bottom - 1;
		return abs$2(top / bottom);
	},
	projectionratio: function(t$1, n) {
		if (n !== 2 && n !== 3) return false;
		if (typeof t$1 === "undefined") t$1 = .5;
		else if (t$1 === 0 || t$1 === 1) return t$1;
		const top = pow$1(1 - t$1, n), bottom = pow$1(t$1, n) + top;
		return top / bottom;
	},
	...
	roots: function(points, line) {
		...
		if (order === 2) {
			...
			if (d$1 !== 0) {
				const m1 = -sqrt$2(b$1 * b$1 - a$1 * c$1), m2 = -a$1 + b$1, v1$1 = -(m1 + m2) / d$1, v2 = -(-m1 + m2) / d$1;
				return [v1$1, v2].filter(reduce$1);
			} else if (b$1 !== c$1 && d$1 === 0) return [(2 * b$1 - c$1) / (2 * b$1 - 2 * c$1)].filter(reduce$1);
			return [];
		}
		...
		if (utils.approximately(d, 0)) {
			if (utils.approximately(a, 0)) {
				...
				return [-c / b].filter(reduce$1);
			}
			const q$1 = sqrt$2(b * b - 4 * a * c), a2 = 2 * a;
			return [(q$1 - b) / a2, (-b - q$1) / a2].filter(reduce$1);
		}
		a /= d;
		b /= d;
		c /= d;
		...
		let u1, v1, x1, x2, x3;
		if (discriminant < 0) {
			const mp3 = -p / 3, mp33 = mp3 * mp3 * mp3, r = sqrt$2(mp33), t$1 = -q / (2 * r), cosphi = t$1 < -1 ? -1 : t$1 > 1 ? 1 : t$1, phi = acos$1(cosphi), crtr = crt(r), t1 = 2 * crtr;
			...
			u1 = crt(-q2 + sd);
			v1 = crt(q2 + sd);
			return [u1 - v1 - a / 3].filter(reduce$1);
		}
	},
	droots: function(p) {
		if (p.length === 3) {
			...
			if (d !== 0) {
				const m1 = -sqrt$2(b * b - a * c), m2 = -a + b, v1 = -(m1 + m2) / d, v2 = -(-m1 + m2) / d;
				return [v1, v2];
...
var PolyBezier = class PolyBezier {
	...
	bbox() {
		const c = this.curves;
		var bbox$6 = c[0].bbox();
		for (var i = 1; i < c.length; i++) utils.expandbox(bbox$6, c[i].bbox());
		return bbox$6;
	}
	offset(d) {
		const offset$1 = [];
		this.curves.forEach(function(v) {
			offset$1.push(...v.offset(d));
		});
...
const { abs: abs$1, min, max, cos: cos$1, sin: sin$1, acos, sqrt: sqrt$1 } = Math;
const pi$2 = Math.PI;
/**
...
var Bezier = class Bezier {
	constructor(coords) {
		...
		if (typeof args[0] === "object") {
			...
			args.forEach(function(point$2) {
				[
					...
					if (typeof point$2[d] !== "undefined") newargs.push(point$2[d]);
				});
			});
			args = newargs;
		}
		...
		this._linear = aligned.reduce((t$1, p) => t$1 + abs$1(p.y), 0) < baselength / 50;
		this._lut = [];
		this._t1 = 0;
		this._t2 = 1;
		this.update();
	...
	raise() {
		...
		for (let i = 1, pi$4, pim; i < k; i++) {
			pi$4 = p[i];
			pim = p[i - 1];
			np[i] = {
				x: (k - i) / k * pi$4.x + i / k * pim.x,
				y: (k - i) / k * pi$4.y + i / k * pim.y
			};
		}
		np[k] = p[k - 1];
	...
	__normal3(t$1) {
		const r1 = this.derivative(t$1), r2 = this.derivative(t$1 + .01), q1 = sqrt$1(r1.x * r1.x + r1.y * r1.y + r1.z * r1.z), q2 = sqrt$1(r2.x * r2.x + r2.y * r2.y + r2.z * r2.z);
		r1.x /= q1;
		r1.y /= q1;
		r1.z /= q1;
		r2.x /= q2;
...
function length$3(aOrLine, pointB) {
	if (isPolyLine(aOrLine)) {
		const sum$4 = aOrLine.reduce((accumulator, v) => length$3(v) + accumulator, 0);
		return sum$4;
	}
...
const guard$4 = (rect, name = `rect`) => {
	...
	if (isPositioned$2(rect)) guard$6(rect, name);
...
* @returns
*/
function distance$1(a, xOrB, y, z) {
...
const nearest$1 = (line, point$1) => {
	const n = (line$1) => {
		...
		const t$1 = Math.min(1, Math.max(0, dot / length$4));
	...
	if (Array.isArray(line)) {
		...
		const dists = pts.map((p) => distance$1(p, point$1));
...
const distanceSingleLine = (line, point$1) => {
	guard$5(line, `line`);
	guard$6(point$1, `point`);
	if (length$3(line) === 0) return length$3(line.a, point$1);
...
function findMinimum(comparer, ...points) {
	...
	let min$1 = points[0];
	for (const p of points) if (isPoint3d(min$1) && isPoint3d(p)) min$1 = comparer(min$1, p);
	else min$1 = comparer(min$1, p);
	return min$1;
}
...
* @returns
*/
const divide$3 = (line, point$1) => Object.freeze({
	...line,
	a: divide$4(line.a, point$1),
	b: divide$4(line.b, point$1)
});
...
const radianArc = (start, end, clockwise = true) => {
	...
	if (end < s) {
		s = 0;
		end = piPi$5 - start + end;
	}
	let d = end - s;
	if (clockwise) d = piPi$5 - d;
	if (d >= piPi$5) return d % piPi$5;
	return d;
};
/**
* Computes the angle arc between a start and end angle,
...
* @returns
*/
const polarToCartesian = (distance$2, angleRadians, origin = Empty$3) => {
	guard$6(origin);
	return Object.freeze({
...
* @returns
*/
const clampMagnitude$2 = (v, max$1 = 1, min$1 = 0) => {
	let mag = v.distance;
	if (mag > max$1) mag = max$1;
	if (mag < min$1) mag = min$1;
	return Object.freeze({
		...v,
		distance: mag
	});
...
const toCartesian$1 = (ray, origin) => {
	...
	const a = toCartesian$2(ray.offset, ray.angleRadian, o);
	const b = toCartesian$2(ray.offset + ray.length, ray.angleRadian, o);
	return {
		a,
		b
	};
...
const fromLine = (line, origin) => {
	...
	return {
		angleRadian: angleRadian$1(line.b, o),
		offset: distance$1(line.a, o),
		length: distance$1(line.b, line.a),
		origin: o
	};
};
...
const fromPivot = (origin = {
	...
	const a = toCartesian$2(left, radianInvert(angleRadian$2), origin);
	const b = toCartesian$2(right, angleRadian$2, origin);
	return Object.freeze({
		a,
		b
	});
...
* @returns {boolean}
*/
const isEqual$5 = (a, b) => isEqual$6(a.a, b.a) && isEqual$6(a.b, b.b);
...
*/
function clamp$1(a, min$1 = 0, max$1 = 1) {
	if (isPoint3d(a)) return Object.freeze({
		x: clamp(a.x, min$1, max$1),
		y: clamp(a.y, min$1, max$1),
		z: clamp(a.z, min$1, max$1)
	});
	else return Object.freeze({
		x: clamp(a.x, min$1, max$1),
		y: clamp(a.y, min$1, max$1)
...
* @param parameterName
*/
const guard$2 = (circle, parameterName = `circle`) => {
	if (isCirclePositioned(circle)) guard$6(circle, `circle`);
...
const distanceFromExterior$1 = (a, b) => {
	...
	if (isCirclePositioned(b)) return Math.max(0, distanceCenter$1(a, b) - a.radius - b.radius);
		...
		const distribution = distance$1(a, b);
...
const intersections = (a, b) => {
	...
	return [sum$3(centroid$2, intersection), subtract$3(centroid$2, intersection)];
};
...
* @returns
*/
const distanceFromCenter = (rect, pt) => distance$1(center$2(rect), pt);
...
const distanceToCenter = (a, shape) => {
	...
	if (isPoint(shape)) return distance$1(a, shape);
...
* @returns {@link Point}
*/
const interpolate$3 = (amount, a, b, allowOverflow = false) => interpolate$4(amount, a, b, allowOverflow);
...
* @returns
*/
const clampMagnitude$1 = (pt, max$1 = 1, min$1 = 0) => {
	const length$4 = distance$1(pt);
	let ratio = 1;
	if (length$4 > max$1) ratio = max$1 / length$4;
	else if (length$4 < min$1) ratio = min$1 / length$4;
...
const fromPointPolar = (pt, angleNormalisation = ``, origin = EmptyCartesian) => {
	...
	if (angleNormalisation === `unipolar` && direction < 0) direction += piPi$4;
		...
		if (direction > pi$1) direction -= piPi$4;
		else if (direction <= -pi$1) direction += piPi$4;
	}
	return Object.freeze({
		distance: distance$1(pt),
...
* @returns
*/
const fromLineCartesian = (line) => subtract$3(line.b, line.a);
/**
...
const fromLinePolar = (line) => {
	guard$5(line, `line`);
	const pt = subtract$3(line.b, line.a);
...
* @returns
*/
const toString$2 = (v, digits) => {
	if (isPolar(v)) return toString$5(v, digits);
	else if (isCartesian(v)) return toString$3(v, digits);
	throw new Error(`Expected polar/cartesian vector. Got: ${v}`);
};
/**
...
const dotProduct = (a, b) => {
	if (isPolar(a) && isPolar(b)) return dotProduct$2(a, b);
...
* @returns
*/
const clampMagnitude = (v, max$1 = 1, min$1 = 0) => {
	if (isPolar(v)) return clampMagnitude$2(v, max$1, min$1);
	else if (isCartesian(v)) return clampMagnitude$1(v, max$1, min$1);
...
const relation = (a, b) => {
	...
	const update = (aa, bb) => {
		...
		totalY += p.y;
		count++;
		const distanceFromStart = distance$1(p, start);
		const distanceFromLast = distance$1(p, lastPoint);
...
var PointTracker = class extends ObjectTracker {
	...
	*/
	distanceFromStart() {
		const initial = this.initial;
		return this.values.length >= 2 && initial !== void 0 ? distance$1(initial, this.last) : 0;
	}
	/**
...
* @returns
*/
const withinRange$1 = (a, b, maxRange) => {
	guard$6(a, `a`);
	guard$6(b, `b`);
	...
	} else guard$6(maxRange, `maxRange`);
...
const multiply$1 = (line, point$1) => Object.freeze({
	...line,
	a: multiply$3(line.a, point$1),
	b: multiply$3(line.b, point$1)
});
...
const relativePosition$1 = (line, pt) => {
	const fromStart = distance$1(line.a, pt);
	const total = length$3(line);
...
* @returns
*/
const rotate$1 = (line, amountRadian, origin) => {
	...
	return Object.freeze({
		...line,
		a: rotate$2(line.a, amountRadian, origin),
		b: rotate$2(line.b, amountRadian, origin)
	});
};
...
const subtract$1 = (line, point$1) => Object.freeze({
	...line,
	a: subtract$3(line.a, point$1),
	b: subtract$3(line.b, point$1)
});
...
const sum$1 = (line, point$1) => Object.freeze({
	...line,
	a: sum$3(line.a, point$1),
	b: sum$3(line.b, point$1)
});
...
function toString$1(a, b) {
	...
	return toString$3(a) + `-` + toString$3(b);
}
...
* @returns
*/
const apply = (line, fn) => Object.freeze({
	...line,
	a: fn(line.a),
	b: fn(line.b)
});
...
* @returns
*/
const normaliseByRect = (line, width, height$3) => Object.freeze({
	...line,
	a: normaliseByRect$1(line.a, width, height$3),
	b: normaliseByRect$1(line.b, width, height$3)
});
/**
...
*/
const withinRange = (line, point$1, maxRange) => {
	const calculatedDistance = distance(line, point$1);
...
const scaleFromMidpoint = (line, factor) => {
	const a = interpolate$4(factor / 2, line);
	const b = interpolate$4(.5 + factor / 2, line);
	return {
		a,
		b
	};
...
const toPath$3 = (line) => {
	...
	return Object.freeze({
		...
		toPoints: () => [a, b],
		rotate: (amountRadian, origin) => toPath$3(rotate$1(line, amountRadian, origin)),
		nearest: (point$1) => nearest$1(line, point$1),
		sum: (point$1) => toPath$3(sum$1(line, point$1)),
		divide: (point$1) => toPath$3(divide$3(line, point$1)),
		...
		apply: (fn) => toPath$3(apply(line, fn)),
		kind: `line`
	});
};
...
const init = (paths, opts = {}) => {
	...
	const checkUnordered = (pt) => {
		const results = paths.map((p, index) => {
			...
			const distance$2 = distance$1(pt, nearest$2);
...
const randomPoint$2 = (within, opts = {}) => {
	...
	switch (strategy) {
		case `naive`: return sum$3(offset$1, toCartesian$2(rand() * radius, rand() * piPi$3));
		case `uniform`: return sum$3(offset$1, toCartesian$2(Math.sqrt(rand()) * radius, rand() * piPi$3));
...
* @param name
*/
const guard$1 = (t$1, name = `t`) => {
	if (t$1 === void 0) throw new Error(`{$name} undefined`);
	guard$6(t$1.a, name + `.a`);
	guard$6(t$1.b, name + `.b`);
	guard$6(t$1.c, name + `.c`);
};
/**
...
* @returns
*/
const isEmpty = (t$1) => isEmpty$3(t$1.a) && isEmpty$3(t$1.b) && isEmpty$3(t$1.c);
/**
...
* @returns
*/
const isEqual$3 = (a, b) => isEqual$6(a.a, b.a) && isEqual$6(a.b, b.b) && isEqual$6(a.c, b.c);
...
function* interiorIntegerPoints(circle) {
	...
	for (let x = xMin; x < xMax; x++) for (let y = yMin; y < yMax; y++) {
		const r = Math.abs(distance$1(circle, x, y));
...
* @returns
*/
const length$1 = (circle) => circumference(circle);
...
* @returns
*/
const interpolate$2 = (circle, t$1) => pointOnPerimeter(circle, t$1 * piPi$1);
...
function multiplyScalar$1(a, value) {
	if (isCirclePositioned(a)) {
		const pt = multiplyScalar$2(a, value);
		return Object.freeze({
			...a,
			...pt,
...
const toPath$2 = (circle) => {
	...
	return {
		...
		interpolate: (t$1) => interpolate$2(circle, t$1),
		bbox: () => bbox$3(circle),
...
* @returns
*/
const interpolate$1 = (paths, t$1, useWidth, dimensions) => {
...
const fromPaths = (...paths) => {
	...
	return Object.freeze({
		...
		interpolate: (t$1, useWidth = false) => interpolate$1(paths, t$1, useWidth, dims),
...
const createArray = (initialValue, rowsOrGrid, columns$1) => {
	...
	for (let index = 0; index < total; index++) t$1[index] = initialValue;
	return t$1;
};
/**
...
const randomLogic = () => {
	return {
		getNeighbours: (grid, cell) => {
			const t$1 = [];
			for (const c of cells(grid, cell)) t$1.push([`n`, c]);
			return t$1;
		},
...
const quadraticSimple = (start, end, bend = 0) => {
	...
	const handle = interpolate$4(Math.abs(bend), middle, target);
...
const distanceFromPointToLine = (p, index, index_) => {
	const lineLength = distance$1(index, index_);
	if (lineLength == 0) return distance$1(p, index);
	const t$1 = ((p.x - index.x) * (index_.x - index.x) + (p.y - index.y) * (index_.y - index.y)) / lineLength;
	if (t$1 < 0) return distance$1(p, index);
	if (t$1 > 1) return distance$1(p, index_);
	return distance$1(p, {
...
const guard = (arc) => {
	...
	if (isPositioned(arc)) guard$6(arc, `arc`);
...
* @returns Length
*/
const length = (arc) => piPi$5 * arc.radius * ((arc.startRadian - arc.endRadian) / piPi$5);
/**
...
* @returns Distance
*/
const distanceCenter = (a, b) => distance$1(a, b);
/**
...
* @returns
*/
const barycentricCoord = (t$1, a, b) => {
...
const fromRadius = (origin, radius, opts = {}) => {
	...
	const points = angles$1.map((a) => toCartesian$2(radius, a, origin));
...
const lengths = (t$1) => {
	guard$1(t$1);
	return [
		distance$1(t$1.a, t$1.b),
		distance$1(t$1.b, t$1.c),
		distance$1(t$1.c, t$1.a)
	];
};
...
const innerCircle = (t$1) => {
	const c = centroid(t$1);
	const p = perimeter$3(t$1) / 2;
	const a = area$3(t$1);
	const radius = a / p;
	return {
		radius,
...
const outerCircle = (t$1) => {
	const [a, b, c] = edges(t$1).map((l) => length$3(l));
...
const rotate = (triangle, amountRadian, origin) => {
	...
	return Object.freeze({
		...triangle,
		a: rotate$2(triangle.a, amountRadian, origin),
		b: rotate$2(triangle.b, amountRadian, origin),
		c: rotate$2(triangle.c, amountRadian, origin)
	});
};
/**
...
const legHeights = (t$1) => {
	const b = baseAngle(t$1);
	return t$1.base * Math.sin(b);
};
...
//#endregion
export { Bezier, arc_exports, bezier_exports, circle_exports, compound_path_exports, curve_simplification_exports, degreeArc, degreeToRadian, degreesSum, ellipse_exports, geometry_exports, grid_exports, layout_exports, line_exports, path_exports, point_exports, polar_exports, quad_tree_exports, radianArc, radianInvert, radianToDegree, radiansFromAxisX, radiansSum, rect_exports, scale, scaler, scaler$1, shape_exports, surface_points_exports, triangle_exports, vector_exports, waypoint_exports, wrap$3 as wrap };
//# sourceMappingURL=geometry-B2sz8eRs.js.map